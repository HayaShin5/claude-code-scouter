...
ステータスバーアイテムをクリックした時、フルコマンドと判定理由を翻訳済みテキストで表示しなければならない（MUST）。判定理由の表示には翻訳ファイルの `patterns` セクションから取得したテキストを使用しなければならない（MUST）。

#### Scenario: クリック時に翻訳済み詳細が表示される
- **WHEN** ユーザーがステータスバーの危険度表示をクリックし、言語が `"ja"` である
- **THEN** フルコマンド文字列と日本語の判定理由を含むポップアップが表示される

#### Scenario: クリック時に英語の詳細が表示される
- **WHEN** ユーザーがステータスバーの危険度表示をクリックし、言語が `"en"` である
- **THEN** フルコマンド文字列と英語の判定理由を含むポップアップが表示される

#### Scenario: デフォルトレベルの場合の詳細
- **WHEN** ユーザーがデフォルトレベルのステータスバー表示をクリックする
- **THEN** フルコマンドと翻訳済みのデフォルト説明文（`ui.defaultSummary`）が表示される

### Requirement: 高危険度時の警告通知
危険度がLv.4以上の場合、VSCodeの警告通知を翻訳済みテキストで自動表示しなければならない（MUST）。

#### Scenario: Lv.4で翻訳済み警告通知が表示される
- **WHEN** 状態ファイルに `level: 4, command: "git push origin main"` が書き込まれ、言語が `"ja"` である
- **THEN** 日本語の警告テキストとコマンド全文を含む通知が表示される

#### Scenario: Lv.5で翻訳済み警告通知が表示される
- **WHEN** 状態ファイルに `level: 5, command: "sudo rm -rf /"` が書き込まれる
- **THEN** 翻訳済みテキストでコマンド全文と危険度レベルを含む警告が表示される
...
### Requirement: 初期状態の表示
拡張の起動時、状態ファイルが存在しない場合は翻訳済みの待機状態テキストを表示しなければならない（MUST）。

#### Scenario: 状態ファイルが存在しない場合
- **WHEN** 拡張がactivateされ、`/tmp/claude-danger-state.json` が存在せず、言語が `"ja"` である
- **THEN** ステータスバーに日本語の待機状態テキスト（`ui.waiting`）が表示される

#### Scenario: 状態ファイルが既に存在する場合
- **WHEN** 拡張がactivateされ、状態ファイルが既に存在する
- **THEN** ファイルの内容を読み込み、対応する危険度表示を翻訳済みテキストで行う

#### Scenario: ツールチップも翻訳される
- **WHEN** ステータスバーアイテムにホバーし、言語が `"ja"` である
- **THEN** 日本語のツールチップテキスト（`ui.waitingTooltip` または翻訳済みパターン説明）が表示される
